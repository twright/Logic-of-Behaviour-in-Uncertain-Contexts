...
{
	printf("Construction ContinuousSystem\n");
}
...
{
	printf("Constructing constinuous system ODE input\n");
...
{
	printf("Constructing constinuous system ODE input from strings\n");
	strOde = strOde_input;
...
ContinuousSystem::~ContinuousSystem()
{
	printf("destroy ContinuousSystem\n");
	hfOde.clear();
...
int ContinuousSystem::reach_low_degree(std::list<Flowpipe> & flowpipes, std::list<int> & flowpipes_safety, long & num_of_flowpipes,
		...
		const std::vector<PolynomialConstraint> & unsafeSet, const bool bSafetyChecking, const bool bPlot, const bool bDump) const
{
	printf("running reach [1]...\n");
	std::vector<Interval> step_exp_table, step_end_exp_table;

	construct_step_exp_table(step_exp_table, step_end_exp_table, step, 2*order);
	std::vector<PolynomialConstraint> dummy_invariant;

	printf("running reach [2]...\n");
...
{
	printf("assign ContinuousSystem\n");
	if(this == &system)
		return *this;
...
{
	printf("creating ContinuousReachability\n");
	bPlot = false;
	bDump = true;
}
...
ContinuousReachability::~ContinuousReachability()
{
	printf("destroying ContinuousReachability\n");
	outputAxes.clear();
	flowpipes.clear();
	orders.clear();
	maxOrders.clear();
...
{
	printf("globalMaxOrder %d", globalMaxOrder);
	printf("running [0]...");
...
void ContinuousReachability::plot_2D(const bool bProjected)
{
	...
	// printf("%d %d %d", plotFormat, PLOT_GNUPLOT, PLOT_MATLAB);
