...
def to_signal(f, fprime, domain):  # , theta=0.01, abs_inf=0.0001):
    ...
    for root in isolate_roots(f, fprime, domain):
        if a < root.lower('RNDD'):
            ...
            print "  I  = {}\nf(I) = {}".format(RIF(I).str(style='brackets'),
...
class Signal(object):
    ...
    #     d = self.domain.absolute_diameter()
    #     a,b = self.domain.endpoints()
    #
    #     I = RIF(f(self.domain))
    #     print 'I      =', self.domain.str(style='brackets')
    #     h = g(ctx(I))
    #     print 'C || I =', RIF(ctx(I)).str(style='brackets')
    #     print 'h      =', h
    #     hprime = h.derivative()
    #     print "h'     =", hprime
    #     sig = to_signal(h, hprime, RIF(0,d))
    #     res = sig(0)
    #
    #     if res is None and d > epsilon:
    #         J, K = self.domain.bisection()
    #         print "bisecting {} -> {}, {}".format(
    #             self.domain.str(style='brackets'),
    #             J.str(style='brackets'),
    #             K.str(style='brackets'),
    #         )
    #         return self.to_domain(J).C(ctx, f, g, epsilon).union(
    #             self.to_domain(K).C(ctx, f, g, epsilon))
    #     #print sig
    #     #print sig(0)
    #     #print bool(sig)
    #     return Signal(self.domain,
    #                   [(self.domain, res)])
...
def C(I, ctx, phi, f, g, epsilon=1):
    ...
    #     import traceback
    #     print "WARNING -- error when exaluating context:"
    #     traceback.print_exc()
    #     res = None
    # return Signal(I, [])
